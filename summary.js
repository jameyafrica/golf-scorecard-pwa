// summary.js — Step 4: End-of-Round Summary Engine
// Reads the in-memory `holes` array handed off from scorecard.js.
// Nothing here persists yet — "Save Round" is a stub until Step 5 wires
// it to IndexedDB.

(function () {

  function classify(hole) {
    if (hole.strokes == null) return null; // hole not yet played
    const diff = hole.strokes - hole.par;
    if (diff <= -1) return 'birdie';   // Birdies & Eagles grouped together
  ...
  function render(courseName, holeCount, holes) {
    ...
    root.innerHTML = `
      ...
      <button type="button" class="save-round-btn" id="save-round-btn" disabled title="Coming in Step 5">
